...
namespace RC
{
    ...
    auto static setup_lua_global_functions_internal(const LuaMadeSimple::Lua& lua, Mod::IsTrueMod is_true_mod) -> void
    {
        ...
        lua.register_function("websocket_init", [](const LuaMadeSimple::Lua& lua) -> int {
            StringType formatted_string = STR("[websocket] ");

            RC::LuaWebSocket module_directory = UE4SSProgram::get_program().get_websocket();
            if (module_directory.isReady())
            {
                return 1;
            }
            module_directory.init();
            formatted_string.append(STR("websocket_init\n"));
            Output::send(formatted_string);
            return 1;
        });
        
        lua.register_function("websocket_status", [](const LuaMadeSimple::Lua& lua) -> int {
            StringType formatted_string = STR("[websocket] ");
            ...
            {
                lua.set_bool(true);
                formatted_string.append(STR("websocket_status true\n"));
            }
            else
            {
                lua.set_bool(false);
                formatted_string.append(STR("websocket_status false\n"));
            }
        ...
        lua.register_function("websocket_receive_message", [](const LuaMadeSimple::Lua& lua) -> int {
            StringType formatted_string = STR("[websocket] ");
            ...
            module_directory.receive_message();
            std::list<std::string> messageList = module_directory.receiveMessage();

            Output::send(STR("websocket_receive_message messageList.empty() {}\n"), messageList.empty());
            if (!messageList.empty())
            {
                LuaMadeSimple::Lua::Table table = lua.prepare_new_table(messageList.size());

                for (size_t count{}; const auto& str : messageList)
                {
                    ++count;
            ...
            module_directory.clearMessage();

            return 1;
        });

        lua.register_function("websocket_send", [](const LuaMadeSimple::Lua& lua) -> int {
            StringType formatted_string = STR("[websocket] websocket_send");
            std::string send_string;

            int32_t stack_size = lua.get_stack_size();
            for (int32_t i = 1; i <= stack_size; i++)
            {
                auto lua_str = std::string(luaL_tolstring(lua.get_lua_state(), i, nullptr));
                auto lua_str1 = to_generic_string(luaL_tolstring(lua.get_lua_state(), i, nullptr));

                if (i > 1)
                {
                    break;
                }
                formatted_string.append(lua_str1);
                send_string.append(lua_str);
            ...
            if (module_directory.isReady())
            {
                module_directory.sendMessage(send_string);
            }
